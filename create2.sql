...
-- criar as tabelas de acordo com a sua ordem de dependência
...
CREATE TABLE QUARTO (
    ...
    FOREIGN KEY(id_tipo_de_quarto) REFERENCES TIPO_DE_QUARTO(id_tipo_de_quarto) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE FIDELIDADE2 (
    ...
    const_estadia INTEGER NOT NULL,
    data_adesão DATE,
...
CREATE TABLE HÓSPEDE (
    ...
    FOREIGN KEY(nº_quarto) REFERENCES QUARTO(nº_quarto) ON UPDATE CASCADE ON DELETE SET NULL,
    FOREIGN KEY(id_fidelidade) REFERENCES FIDELIDADE2(id_fidelidade) ON UPDATE CASCADE ON DELETE SET NULL
);
...
CREATE TABLE TIPO_DE_RESERVA (
    id_tipo_de_reserva INTEGER,
    nome_tipo_de_reserva VARCHAR(20) NOT NULL CHECK(nome_tipo_de_reserva = 'VIP' OR nome_tipo_de_reserva = 'Normal'),
    PRIMARY KEY(id_tipo_de_reserva)
);
...
CREATE TABLE RESERVA (
    ...
    id_tipo_de_reserva INTEGER NOT NULL,
    PRIMARY KEY(id_reserva),
    FOREIGN KEY(id_tipo_de_reserva) REFERENCES TIPO_DE_RESERVA(id_tipo_de_reserva) ON UPDATE CASCADE ON DELETE CASCADE
);
